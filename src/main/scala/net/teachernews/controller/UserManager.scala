...
class UserManager {
  ...
  def getTitleOptions: Array[SelectItem] =
    for (titleValue <- Title.values) yield new SelectItem(titleValue, bundle.getString(titleValue.toString))

  /**
   * @return available RoleType-Enum values (localized)
   */
  def getRoleOptions: Array[SelectItem] =
    for (pStatusValue <- RoleType.values) yield new SelectItem(pStatusValue, bundle.getString(pStatusValue.toString))
